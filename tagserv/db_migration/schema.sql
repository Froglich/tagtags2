...
INSERT INTO tagtags(schema_version) VALUES (4);
...
CREATE TABLE sessions (
    ...
    expires TIMESTAMP NOT NULL DEFAULT (NOW() + '1 day'::interval)
);
...
CREATE TABLE data (
    ...
    value TEXT NOT NULL,
    modified NUMERIC NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW()),
    synctime NUMERIC NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())
);
...
CREATE VIEW view_projects AS SELECT
    ...
    COALESCE((SELECT to_timestamp(MAX(modified))::text FROM data d WHERE d.project = p.project), 'never') last_modified
FROM projects p;
...
CREATE VIEW view_project_identifiers AS SELECT
    ...
    COALESCE((SELECT to_timestamp(MAX(modified))::text FROM data da WHERE da.project = d.project AND da.identifier = d.identifier), 'never') last_modified
...
CREATE VIEW view_project_access AS SELECT
    ...
    CASE
        WHEN u.full_access
        THEN TRUE
        WHEN (SELECT COUNT(*) FROM project_groups pg LEFT JOIN user_groups ug ON ug.group_id = pg.group_id WHERE pg.project = p.project AND ug.user_id = u.user_id AND pg.can_modify = TRUE) > 0
        THEN TRUE
        WHEN u.user_id = p.created_by
        THEN TRUE
        ELSE FALSE
